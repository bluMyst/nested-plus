...
var FEMALE, INVALID_GENDER, Instance, MALE, Thing, bookCase, chance, checkMissingThings, choose, cleanThings, debug, iN, instances, isotopeGenerator, launchNest, miscGenerators, nameGenerators, randint, things, thingsN, thoughtGenerators, toggle, weightedChoose,
...
Thing = (function() {
  function Thing(name_, contains1, namegen) {
    this.name_ = name_;
    this.contains = contains1;
...
checkMissingThings = function() {
  var allContents, allMissing, i, i2, i3, ref, str, thisContent, thisThing, v3;
  allContents = [];
  allMissing = [];
  for (i in things) {
    thisThing = things[i];
    ref = thisThing.contains;
    for (i2 in ref) {
      thisContent = ref[i2];
      if (typeof thisContent !== 'string') {
        for (i3 in thisContent) {
          v3 = thisContent[i3];
          allContents.push(v3);
        }
      } else {
  ...
  for (i in allContents) {
    ...
    if (!things[thisContent] && thisContent !== '') {
...
cleanThings = function() {
  var i, iT, newContains, thisT, toConcat;
  for (iT in things) {
    thisT = things[iT];
    toConcat = [];
    for (i in thisT.contains) {
      if (typeof thisT.contains[i] === 'string') {
        if (thisT.contains[i][0] === '.') {
          if (things[thisT.contains[i].slice(1)] !== void 0) {
            toConcat = toConcat.concat(things[thisT.contains[i].slice(1)].contains);
          }
          thisT.contains[i] = '';
        }
      }
...
weightedChoose = function(arr, weightChoose) {
  if (weightChoose <= 0 || weightChoose === void 0) {
    weightChoose = 1;
  }
...
isotopeGenerator = function(protons, electrons, naturalAbundances) {

  /*
   * example naturalAbundances:
   * [[0.99, 16], [0.01, 15] 16 neutrons has 99% probability
   */
  var j, len, neutrons, prob, ref, sum;
  sum = 0;
  for (j = 0, len = naturalAbundances.length; j < len; j++) {
    ref = naturalAbundances[j], prob = ref[0], neutrons = ref[1];
    sum += prob;
  }
  if (sum !== 1) {
    throw "isotopeGenerator called with invalid naturalAbundances: probabilities don't add up to 1.";
  }
  return function() {
    var k, len1, r, ref1;
    r = Math.random();
    sum = 0;
    for (k = 0, len1 = naturalAbundances.length; k < len1; k++) {
      ref1 = naturalAbundances[k], prob = ref1[0], neutrons = ref1[1];
      sum += prob;
      if (r <= sum) {
        return ["proton," + protons, "neutron," + neutrons, "electron," + electrons];
      }
    }
  };
};
...
new Thing('sulfur', ['proton,16', 'neutron,32', 'electron,16']);
...
nameGenerators = {
  modern: function(gender) {
    ...
    if (gender === MALE && randint(0, 2000) === 1) {
      str = choose(['Elvis Presley', 'Gabe Newell']);
    }
    return str;
  },
  medieval: function(gender) {
    var str;
    if (gender === FEMALE) {
      str = choose(['Millicent', 'Alinor', 'Eleanor', 'Agnes', 'Alice', 'Avice', 'Beatrice', 'Cecily', 'Emma', 'Isabella', 'Joan', 'Juliana', 'Margery', 'Matilda', 'Roh', 'Morgan', 'Elizabeth', 'Kethleen']);
    } else if (gender === MALE) {
      str = choose(['Adam', 'Geoffrey', 'Gilbert', 'Henry', 'Hugh', 'John', 'Nicholas', 'Peter', 'Ralf', 'Richard', 'Robert', 'Roger', 'Simon', 'Thomas', 'Walter', 'William', 'Robin', 'Albin', 'Bayard', 'Erwin']);
    }
    str += ' ' + choose(['Strong', 'Tall', 'Grand', 'Bold', 'Big', 'Small', 'Fine', 'Good', 'Glad', 'Green', 'Blue', 'Red', 'Black', 'White', 'Pale', 'Gray', 'Gold', 'Silver', 'Dark', 'Light', 'Brave', 'Sly']) + choose(['ington', 'son', 'house', 'door', 'castle', 'forest', 'tree', 'leaf', 'wind', 'rain', 'snow', 'rock', 'stone', 'river', 'sea', 'ship', 'smith', 'craft', 'cook', 'worth', 'might', 'wolf', 'bear', 'sheep', 'pig', 'fox', 'hunt', 'dragon']);
    return str;
  },
  ancient: function(gender) {
    var str;
    ...
    if (gender === MALE && randint(0, 250) === 1) {
      str = 'Dave';
    }
    return str;
  },
  future: function(gender) {
    var str;
    if (gender === FEMALE) {
      str = choose(['Alice', 'Trillian', 'Zeria', 'Zinda', 'Myriad', 'Shosset', 'Tary', 'Wayt', 'Cassidy', 'Millanda', 'Replika', 'Simia', 'Mirrix', 'Bariona', 'Sossis', 'Kroassent', 'Five', 'Seven', 'Fourteen']);
    } else if (gender === MALE) {
      str = choose(['Bob', 'Zaphod', 'Zorvax', 'Zerebius', 'Borbon', 'Bradbury', 'Isaac', 'Werber', 'Omni', 'Robion', 'Shossur', 'Alistor', 'Clonos', 'Sorbel', 'Boodin', 'Bagget', 'Three', 'Twelve', 'Twenty']);
    }
...
miscGenerators = {
  painting: function() {
    var adjs, locs, objs, str, tverbings, verbings;
    ...
    str = str.replace(/\$verbing2/g, choose(verbings)).replace(/\$verbing/g, choose(verbings)).replace(/\$tverbing/g, choose(tverbings)).replace(/\$loc/g, choose(locs)).replace(/\$adj2/g, choose(adjs)).replace(/\$obj2/g, choose(objs)).replace(/\$adj/g, choose(adjs)).replace(/\$obj/g, choose(objs));
    return str;
  },
  note: function() {
    var str;
...
Instance = (function() {
  ...
  Instance.prototype.name = function() {
    var gender, nameParts, nameToString;
    this.name_ = this.type.namegen;
    nameToString = function(name) {
      var i, j, len, str;
    ...
    this.name_ = nameToString(this.name_);
    nameParts = this.name_.split('|');
    this.name_ = nameParts[0];
    if (this.name_.search(/^\*.*\*$/) === -1) {
      gender = INVALID_GENDER;
    } else if (this.name_.search(/PERSON\*$/) !== -1) {
      gender = choose([MALE, FEMALE]);
    } else if (this.name_.search(/WOMAN\*$/) !== -1) {
      gender = FEMALE;
    } else if (this.name_.search(/MAN\*$/) !== -1) {
      gender = MALE;
    } else {
      gender = INVALID_GENDER;
    }
    switch (this.name_) {
      ...
      case '*MAN*':
      case '*WOMAN*':
        this.name_ = nameGenerators.modern(gender);
        break;
      case '*MEDIEVAL PERSON*':
      case '*MEDIEVAL MAN*':
      case '*MEDIEVAL WOMAN*':
        this.name_ = nameGenerators.medieval(gender);
        break;
      case '*ANCIENT PERSON*':
      case '*ANCIENT MAN*':
      case '*ANCIENT WOMAN*':
        this.name_ = nameGenerators.ancient(gender);
        break;
      case '*FUTURE PERSON*':
      case '*FUTURE MAN*':
      case '*FUTURE WOMAN*':
        this.name_ = nameGenerators.future(gender);
        break;
      case '*MEMORY*':
        this.name_ = thoughtGenerators.memory();
        break;
      case '*SADTHOUGHT*':
        this.name_ = thoughtGenerators.sadThought();
        break;
      case '*HAPPYTHOUGHT*':
        this.name_ = thoughtGenerators.happyThought();
        break;
      case '*MEDIEVAL MEMORY*':
        this.name_ = thoughtGenerators.medievalMemory();
        break;
      case '*MEDIEVAL THOUGHT*':
        this.name_ = thoughtGenerators.medievalThought();
        break;
      case '*ANCIENT MEMORY*':
        this.name_ = thoughtGenerators.ancientMemory();
        break;
      case '*ANCIENT THOUGHT*':
        this.name_ = thoughtGenerators.ancientThought();
        break;
      case '*FUTURE MEMORY*':
        this.name_ = thoughtGenerators.futureMemory();
        break;
      case '*FUTURE THOUGHT*':
        this.name_ = thoughtGenerators.futureThought();
        break;
      case '*PAINTING*':
        this.name_ = miscGenerators.painting();
        break;
      case '*NOTE*':
        this.name_ = miscGenerators.note();
        break;
      case '*BOOK*':
        this.name_ = bookCase(miscGenerators.book());
        break;
      case '*CHAR*':
        this.name_ = miscGenerators.char();
        break;
      case '*MONUMENT*':
        this.name_ = bookCase(miscGenerators.monument());
    }
  ...
  Instance.prototype.grow = function() {
    var New, contains, generateContent, i, ii, makeAmount, makeProb, toMake;
    if (this.grown === true) {
      return;
    }
    this.name();
    if (typeof this.type.contains === 'function') {
      contains = this.type.contains();
    } else {
      contains = this.type.contains;
    }
    for (i in contains) {
      toMake = contains[i];
      generateContent = function(contents) {
...
bookCase = function(name) {
  var capitalizeTitle, i, word, wordsToNotCapitalize;
  capitalizeTitle = function(words) {
    return words[0].toUpperCase() + words.slice(1);
  };
  wordsToNotCapitalize = ['of', 'in', 'on', 'and', 'the', 'an', 'a', 'with', 'to', 'for', 'from', 'be', 'is'];
  name = name.split(' ');
  for (i in name) {
    word = name[i];
    if (i === 0) {
      name[i] = capitalizeTitle(word);
    } else if (indexOf.call(wordsToNotCapitalize, word) < 0) {
...
new Thing('test', function() {
  var contents, thingName;
  contents = [];
  for (thingName in things) {
...
new Thing('test1', (function() {
  return choose([['diamond'], ['oil']]);
}), (function() {
  return 'test1: ' + choose(['foo', 'bar']);
}));

new Thing('test2', function() {
  return choose([
    [
      (function() {
        return 'diamond';
...
new Thing('rock', ['silica', 'aluminum,30%', 'iron,20%', 'potassium,20%', 'sodium,50%', 'calcium,50%']);

new Thing('silica', ['silicon', 'oxygen']);

new Thing('organic molecule', ['carbon,3-10', 'hydrogen,3-10', 'oxygen,0-5', 'nitrogen,0-5']);
...
new Thing('amino acids', ['alanine', 'cysteine', 'aspartic acid', 'glutamic acid', 'phenylalanine', 'glycine', 'histidine', 'isoleucine', 'lysine', 'leucine', 'methionine', 'asparagine', 'pyrrolysine', 'proline', 'glutamine', 'arginine', 'serine', 'threonine', 'selenocysteine', 'valine', 'tryptophan', 'tyrosine']);
...
new Thing('sea monster thought', [], ['IIIIII MUSSST SCREEEAAAM', 'I AMMMM AWAKENED', 'ALLLLLL FEAR MEEEEE', 'NOOOOONE SHALL LIVE', 'I MUSSSSST EATTTTT', 'DEEEEEEEEP I SSSSLUMBER', 'IIIII SHALL CONSSSSUME', 'IIIII SHALL DEVOUUUUURRRRR', 'LIFFFFFFE MUSSSSST PERISHHHHH', 'NNNNNNNNOURISHMENT', 'ALL SHALLLLLLL GO INSSSSSSANE', 'SSSSSSANITY SHALL YIELDDDDD', 'EXXXXXILED I WASSSSS', 'EONSSSSS I HAVE SLUMBERED', 'EONSSSSS I HAVE WAITED', 'MORTALSSSSSS BEHOLDDDDD', 'I COMMMMME FROM DEEP', 'IMMMMMMOBILE I WATCHHHH', 'SSSSSKITTER', 'THEY FFFFFLOAAAAAT']);
...
new Thing('rifle', ['steel', 'aluminum,50%', 'polymers,20%', 'bullet,0-6']);

new Thing('handgun', ['steel', 'aluminum,50%', 'polymers,20%', 'bullet,0-6']);
...
new Thing('cog', [['copper', 'plastic', 'iron', 'steel', 'aluminum']], ['cog', 'gear', 'spur gear', 'helical gear', 'bevel gear', 'harmonic drive', 'spring', 'pump', 'sprocket', 'wheel', 'chain', 'belt', 'track', 'bolts', 'gizmo', 'pulley', 'puffer', 'smoker', 'vent']);
