...
var FEMALE, INVALID_GENDER, Instance, MALE, Thing, bookCase, checkMissingThings, choose, cleanThings, debug, iN, instances, launchNest, make, miscGenerators, nameGenerators, randint, things, thingsN, thoughtGenerators, toggle, weightedChoose,
...
bookCase = function(s) {
  var capitalizeWord, i, ref, wordsToNotCapitalize;
  capitalizeWord = function(s) {
    return s[0].toUpperCase() + s.slice(1);
  };
  wordsToNotCapitalize = ['of', 'in', 'on', 'and', 'the', 'an', 'a', 'with', 'to', 'for', 'from', 'be', 'about', 'is'];
  s = s.split(' ');
  for (i in s) {
    if (i === 0) {
    ...
    if (ref = s[i], indexOf.call(wordsToNotCapitalize, ref) < 0) {
      s[i] = capitalizeWord(s[i]);
    }
  }
  s = s.join(' ');
